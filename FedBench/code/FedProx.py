...
class Worker():
  ...
  def local_train(self):
    acc_train,loss_train = train_fedprox(self.model,args.criterion,self.trainloader,args.local_epochs,args.mu)
...
def train_fedprox(model,criterion,trainloader,epochs,mu):
  if args.dataset_name=='Sent140':
      ...
      for epoch in range(epochs):
        ...
        for (data,labels) in trainloader:
          ...
          fed_prox_reg = 0.0
          for param_index, (new_param, old_param) in enumerate(zip(model.parameters(),old_model.parameters())):
            fed_prox_reg += ((mu / 2) * torch.norm((new_param - old_param))**2)
          loss += fed_prox_reg
...
filename = 'FedProx_{}'.format(args.dataset_name)
result_path = '../result/'
